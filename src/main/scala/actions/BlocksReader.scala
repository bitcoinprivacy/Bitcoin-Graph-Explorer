...
class BlocksReader(args: List[String]) {
  ...
  var totalOutIn = 0
  var vectorMovements: Vector[(Option[Array[Byte]], Option[Array[Byte]], Option[Array[Byte]], Option[Int], Option[Double])] = Vector()
  ...
  def readBlocksfromFile: Long =
    {
      ...
      // TODO: remove remaining "resume" functionality
      if (resuming)
        populateOOOInputMap
