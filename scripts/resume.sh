...
echo "[$(date)] Reading blockchain from "$lastBlockNumber" until "$newBlockNumber

a="JAVA_OPTS=-Xmx1g /usr/bin/scala -classpath /root/bge/target/scala-2.11/Bitcoin\ Graph\ Explorer-assembly-2.0.jar Explorer resume > /root/bge/blockchain/resume.log"
eval $a
